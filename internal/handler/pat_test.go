...
// PAT 在互动服务的端到端回归：真库 + 真路由 + 同形的账号服务内省桩。
...
func TestPATEndToEnd(t *testing.T) {
	...
	reader := patBearer('r')       // 只有发帖码：够读"需登录"的读接口
	poster := patBearer('p')       // 发帖码：过 require(community.post.create)
	outsider := patBearer('o')     // 只有目录码：互动写接口越权
	...
	stub, srv := newPATAuthStub(t, map[string]map[string]any{
		reader: {"valid": true, "user_id": "aaaaaaaa-0000-0000-0000-000000000001", "username": "pat-reader",
			"role": "user", "permissions": []string{auth.PermissionPostCreate}},
		poster: {"valid": true, "user_id": "aaaaaaaa-0000-0000-0000-000000000002", "username": "pat-poster",
			"role": "user", "permissions": []string{auth.PermissionPostCreate}},
		outsider: {"valid": true, "user_id": "aaaaaaaa-0000-0000-0000-000000000003", "username": "pat-outsider",
			"role": "user", "permissions": []string{"catalog.entity.edit"}},
		adminNoScope: {"valid": true, "user_id": "aaaaaaaa-0000-0000-0000-000000000004", "username": "pat-admin",
			"role": "admin", "permissions": []string{}},
		fresh: {"valid": true, "user_id": "aaaaaaaa-0000-0000-0000-000000000005", "username": "pat-fresh",
			"role": "user", "permissions": []string{auth.PermissionPostCreate}},
	})
	...
	router := gin.New()
	New(s, catalog.New("", 0), verifier).Register(router)
	...
	// 1) 读接口（需登录）：PAT 能读到自己的收藏列表，且响应不带 mf_session cookie。
	w := do(http.MethodGet, "/api/favorites/mine", "", reader)
	if w.Code != http.StatusOK {
		t.Fatalf("PAT 读自己的收藏应 200，实际 %d：%s", w.Code, w.Body.String())
	}
	if cookie := w.Header().Get("Set-Cookie"); cookie != "" {
		t.Fatalf("PAT 请求不该产出 cookie: %s", cookie)
	}
	// 2) 越权写：scopes 里没有发帖码 → 403（不是 401，也不是放行）。
	//    探针载荷刻意非法：400 只可能出现在权限闸门放行之后。
	w = do(http.MethodPost, "/api/community/topics", "{}", outsider)
	if w.Code != http.StatusForbidden || !strings.Contains(w.Body.String(), "forbidden") {
		t.Fatalf("scopes 收紧的 PAT 发帖应 403，实际 %d：%s", w.Code, w.Body.String())
	}
	// 3) 持码的 PAT 过闸门（载荷不合由处理器回 400，证明身份进了上下文）。
	w = do(http.MethodPost, "/api/community/topics", "{}", poster)
	...
	w = do(http.MethodPost, "/api/community/topics", "{}", adminNoScope)
	if w.Code != http.StatusForbidden {
		t.Fatalf("scopes 为空的管理员 PAT 不该按角色兜底放行，实际 %d：%s", w.Code, w.Body.String())
	}
	// 5) 缓存命中：同一令牌的第二次请求不再打内省端点（reader 已在上面用过一次）。
	w = do(http.MethodGet, "/api/favorites/mine", "", reader)
	if w.Code != http.StatusOK {
	...
	// 6) 无效 PAT：401 + 稳定机器码（无效/吊销/过期共用一个码）。形态合法所以会问一次内省。
	w = do(http.MethodGet, "/api/favorites/mine", "", patBearer('z'))
	...
	w = do(http.MethodGet, "/api/favorites/mine", "", auth.PATPrefix+"not-a-real-token")
	if w.Code != http.StatusUnauthorized || !strings.Contains(w.Body.String(), auth.CodeInvalidToken) {
	...
	srv.Close()
	w = do(http.MethodGet, "/api/favorites/mine", "", fresh)
	...
	req := httptest.NewRequest(http.MethodGet, "/api/favorites/mine", nil)
	...
	// 累计 5 次 = reader/poster/outsider/adminNoScope 各一次 + 形态合法的未知令牌一次；
